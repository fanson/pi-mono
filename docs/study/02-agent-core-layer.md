...
agentLoopContinue(context, config, signal?, streamFn?)
  ...
  → 要求最后一条消息不是 assistant（即 user 或 toolResult）
  → 返回 EventStream<AgentEvent, AgentMessage[]>
```
...
> **源码对照**: `packages/agent/src/agent-loop.ts` — runLoop L155, streamAssistantResponse L238, prepareToolCallArguments L458, prepareToolCall L472
  ...
  → interactive-mode.ts: session.prompt(text, { streamingBehavior: "steer" })
    → agent-session.ts: _queueSteer(expandedText)
      → agent.steer({ role: "user", content, timestamp })
        → agent.ts: steeringQueue.push(msg)
...
```
1. transformContext(messages)  — 可选，操作 AgentMessage[]
   例如：上下文窗口裁剪、注入外部上下文
...
4. 遍历 stream 事件:
   ...
   - delta: 更新 context 中的 partial，发射 message_update
...
阶段 3: FINALIZE（完成）
  - 调用 afterToolCall 钩子
    → 可以替换 content、details 或 isError
...
- **file-mutation-queue 的意义**：execute 阶段并发，所以两个 edit 同时操作同一文件会竞争（通过 `withFileMutationQueue` 串行化，详见 [05-tool-system-deep-dive.md](05-tool-system-deep-dive.md)）

## Agent 类 (src/agent.ts)

> **源码对照**: `packages/agent/src/agent.ts` — Agent L116, prompt() L400, steer() L311
...
> **源码对照**: `packages/agent/src/types.ts` — AgentMessage L245, AgentTool L273, AgentLoopConfig L96, AgentEvent L298
