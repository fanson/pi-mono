...
agentLoopContinue(context, config, signal)
  ...
  → 要求最后一条消息是 user 或 toolResult（不能是 assistant）
  → 返回 EventStream<AgentEvent, AgentMessage[]>
```
...
> **源码对照**: `packages/agent/src/agent-loop.ts` — runLoop L155, streamAssistantResponse L238, prepareToolCall L458
...
- **file-lock 的意义**：execute 阶段并发，所以两个 edit 同时操作同一文件会竞争

## Agent 类 (src/agent.ts)

> **源码对照**: `packages/agent/src/agent.ts` — Agent L116, prompt() L393, steer() L311
