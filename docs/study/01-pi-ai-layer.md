...
| **OpenAI Responses / OpenAI Completions（直连官方 OpenAI 时）** | `prompt_cache_key` + `prompt_cache_retention` | `providers/openai-responses.ts`、`providers/openai-completions.ts` |
...
这条规则现在同时覆盖 `openai-responses.ts` 和**直连官方 `api.openai.com`** 的 `openai-completions.ts`；如果走第三方代理或 OpenAI-compatible base URL，则不会注入这些字段。
