...
## Issue 4: 工具分区调度仍缺失（只有 coarse executionMode）

### 当前实现

`AgentTool` 现在已经有 `executionMode?: ToolExecutionMode`，而 `executeToolCalls()` 会检查当前批次里是否有串行工具：
...
也就是说：
- Pi 已经不再是“只有全局开关”那么简单；tool-level `executionMode: "sequential"` 会把当前整批拉回串行
- 但它仍然没有 Claude Code 那种每工具 `isConcurrencySafe` / `[并行块] → [串行块] → [并行块]` 分区批处理
- `file-mutation-queue` 只能缓解同文件写入冲突，不等于真正的并发安全调度

### 结论

更准确的说法是：Pi 已经有一个 **coarse 的 tool-level sequential override**，但仍缺少真正的分区调度能力。
所以这更像 feature request / 调度策略讨论，而不是明确 bug，优先级仍然不高。
...
// line 153 左右只是 mimeType 探测；真正把非图片文件当 UTF-8 文本读取的是 186-188 行
