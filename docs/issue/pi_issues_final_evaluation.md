...
经代码路径验证：`bash.ts` 的 `reject(new Error(...))` → `agent-loop.ts` 的 `executePreparedToolCall` catch → 返回 `{ result, isError: true }`。
...
- 这是可靠性问题，不是功能增强
- 一个合理的默认超时（如 10 分钟）不违反 minimal 原则
- 修复极小
...
### Issue 2（兄弟工具失败不取消）、Issue 3（并行无并发上限）、Issue 4（工具并发安全声明）
