...
经代码路径验证：`packages/coding-agent/src/core/tools/bash.ts` 的 `reject(new Error(...))`（约 lines 379-381）→ `packages/agent/src/agent-loop.ts` 的 `executePreparedToolCall` catch（约 lines 555-560）→ 返回 `{ result, isError: true }`。
...
```typescript
// packages/agent/src/agent-loop.ts lines 555-560
} catch (error) {
    return {
...
**为什么不提：** 这条问题需要换个更准确的表述。Pi 现在已经有一个 coarse 的 tool-level `executionMode`：如果某个工具声明 `executionMode: "sequential"`，当前整批调用会回退到串行。但它仍然没有 Claude Code 那种 per-tool 并发安全声明 / 分区批处理。这个差距更像 feature request / 调度策略讨论，不是明确 bug；Pi 也已经用 `file-mutation-queue` 缓解了同文件写冲突，所以不适合作为优先 issue。
