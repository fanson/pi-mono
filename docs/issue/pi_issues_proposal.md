...
**问题定位：** `packages/coding-agent/src/core/tools/bash.ts` 第 360-365 行
...
**问题定位：** `packages/coding-agent/src/core/tools/bash.ts` 第 27-30 行
...
**问题定位：** `packages/agent/src/agent-loop.ts` 第 194-198 行、`packages/ai/src/types.ts` 第 182 行
...
// types.ts 第 182 行 - StopReason 包含 "length"
// stopReason: "length" | "error" | "aborted" | "end_turn" | "tool_use"
```
...
- 截断的工具调用 JSON 无法解析 → 通常在参数验证阶段失败 → 返回错误消息给模型
- 但如果 JSON 恰好在一个合法位置截断（如字符串中间截断后意外闭合），可能执行错误的命令
- 需要验证 Pi 默认的 `maxTokens` 是多少——如果已经设得足够大（如 16K+），此问题很少触发

**建议修复方案（如果要提交）：**
```typescript
// agent-loop.ts - 在 stopReason 检查中增加 "length" 处理
if (message.stopReason === "error" || message.stopReason === "aborted") {
    await emit({ type: "turn_end", message, toolResults: [] });
    await emit({ type: "agent_end", messages: newMessages });
    return;
...
if (message.stopReason === "length") {
    ...
    if (toolCalls.length > 0) {
        ...
        message.content = message.content.filter((c) => 
            c.type !== "toolCall" || c.id !== lastToolCall.id
        );
        // 添加恢复消息
        pendingMessages.push({
            role: "user",
            content: [{ type: "text", text: "Output was truncated. Continue from where you left off." }],
        });
    }
}
```
...
**问题定位：** `packages/coding-agent/src/core/tools/edit.ts` 第 222-246 行

**验证过的代码路径：**
```
edit.ts execute（第 222-246 行）:
...
```typescript
// edit.ts 第 222-246 行（简化）
const rawContent = await ops.readFile(absolutePath);
const { bom, text: content } = stripBom(rawContent);
const normalizedContent = normalizeToLF(content);

// oldText 匹配提供部分保护 - 如果被编辑区域被外部修改了，这里会失败
const { baseContent, newContent } = applyEditsToNormalizedContent(
    normalizedContent, edits, path
);

// 但如果外部修改在 oldText 以外的区域，这里会成功
// → finalContent 包含 newText 替换后的内容
// → 但不包含外部在其他区域的修改（因为 rawContent 是外部修改后的版本）
// ← 等等...实际上 rawContent 包含了外部修改...

// 真正的风险场景：
// T0: Pi 读取文件 (read tool)，模型基于旧内容生成 edit
// T1: 用户在 IDE 中修改了文件的其他部分
// T2: Pi 执行 edit tool，re-read 获取 T1 版本
//     → oldText 在 T1 版本中仍存在 → 匹配成功
//     → 但模型生成的 newText 可能基于 T0 版本的上下文假设
//     → 最终文件包含 T1 的外部修改 + 基于 T0 假设的 newText
```
...
**问题定位：** `packages/agent/src/types.ts` 第 35 行、`packages/agent/src/agent-loop.ts` 第 344-347 行

**验证过的代码路径：**
```
types.ts: ToolExecutionMode = "sequential" | "parallel"  // 全局开关
agent-loop.ts executeToolCalls:
  → if (config.toolExecution === "sequential")
    → executeToolCallsSequential(...)  // 全部串行
  → else
    → executeToolCallsParallel(...)    // 全部并行
```

**核心问题：** 当前并行/串行是全局配置，无法按工具类型区分。用户被迫在"全部串行（慢）"和"全部并行（有风险）"之间选择。

```typescript
// types.ts 第 35 行
export type ToolExecutionMode = "sequential" | "parallel";

// agent-loop.ts 第 344-347 行
if (config.toolExecution === "sequential") {
...
**Claude Code 的做法：**
```typescript
// Tool.ts - 每个工具声明自己的并发安全性
isConcurrencySafe(input: I): boolean {
    // 例如 BashTool：只读命令返回 true，写命令返回 false
}

// toolOrchestration.ts 第 91-116 行 - 按安全性分区
...
```
Claude Code 将工具调用分区：连续的 `isConcurrencySafe: true` 工具并行执行，非安全工具串行执行。例如 5 个 read + 1 个 write + 3 个 read → [5 并行] → [1 串行] → [3 并行]。

**实际影响：**
- 用户选择 `parallel` 模式后，`edit` 和 `write` 也会并行执行，理论上可能导致文件竞争
- Pi 有 `file-mutation-queue` 缓解了同文件的写冲突，但不同文件的 write 仍然并行
- 用户选择 `sequential` 模式后，所有 `read` 也被迫串行，浪费时间

**建议修复方案（如果要提交）：**
```typescript
// types.ts - AgentTool 接口新增
export interface AgentTool<TArgs> {
    name: string;
    execute: (args: TArgs, ...) => Promise<AgentToolResult<any>>;
    isConcurrencySafe?: (args: TArgs) => boolean;  // 新增
}

// agent-loop.ts - 分区执行
function partitionToolCalls(toolCalls: AgentToolCall[], tools: AgentTool<any>[]): Batch[] {
    // 连续的 concurrency-safe 工具合并为一个并行批次
    // 非 safe 工具单独作为串行批次
}
```

**不建议提交的原因：**
- 完全是增强型 feature，不是 bug
- Pi 已有 `file-mutation-queue` 防止同文件并发写入
- 分区算法、`isConcurrencySafe` 接口扩展增加核心复杂度
- 这更适合作为 extension 或未来 feature request
- 改动影响 `AgentTool` 接口、所有工具注册代码、执行调度逻辑

**Issue 标题建议（如果决定提交）：** `tool execution: per-tool concurrency safety instead of global sequential/parallel switch`

**复现步骤：**
1. 启动 `pi` session，默认 `toolExecution: "parallel"`
2. 发送 prompt："读取 file1.ts 和 file2.ts 的内容，然后修改 file3.ts 中的一个函数"
3. 观察模型是否将 read 和 edit 操作全部并行执行
4. 切换为 `toolExecution: "sequential"` 重复测试
5. **预期行为：** read 操作并行，edit 操作串行（智能分区）
6. **实际行为（parallel 模式）：** 全部并行，包括 edit
7. **实际行为（sequential 模式）：** 全部串行，包括 read（不必要的等待）
