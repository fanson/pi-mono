...
**问题定位：** `packages/agent/src/agent-loop.ts` 已经有一个 coarse 的 tool-level sequential 逃生口，但还没有真正的分区调度
...
**核心问题：** 当前已经能让单个工具用 `executionMode: "sequential"` 把整批调用拉回串行，但仍无法像 Claude Code 那样按工具安全性切成 `[并行块] → [串行块] → [并行块]`。用户依然是在“整批并行”和“整批回退串行”之间做粗粒度选择。
...
**更准确的结论：**
- “Pi 完全只有全局 sequential / parallel 开关” 这个判断已经过期
- 更准确地说，Pi 现在只有 coarse 的 sequential override，还没有真正的 per-tool 分区调度
